...
object Utils {
   ...
   * @return
   */
  def generateMap(files: List[String]) =
    files.foldLeft(Map.empty[String,Long]){
      (result, file) => result + (file -> generateCRC(file))
    }
   ...
   *         Or true
   */
  def compareCRCMaps(oldMap: Map[String, Long],
                     newMap: Map[String, Long]
                      ): Either[List[String],Boolean] = {

    val corruptedFiles = newMap.foldLeft(List.empty[String]){
